...
void RigidBody::MoveLeft()
{
	...
	_physic->Speed.x -= _physic->_accelerationSpeed * deltaTime;
	if (_physic->Speed.x < -_physic->_maxSpeedX* deltaTime)
	{
...
void RigidBody::Jump()
{
	...
	Player* player = static_cast<Player*>(GetOwner());
	float angle = player->GetAngle();
	_physic->Speed.x -= _physic->_jumpForce * deltaTime * sin(angle);
...
void RigidBody::SetFriction()
{
	...
	{
		_physic->Speed.x -= _physic->_frictionSpeed * deltaTime;
		if (_physic->Speed.x < 0)
			_physic->Speed.x = 0;
	}
	else 
